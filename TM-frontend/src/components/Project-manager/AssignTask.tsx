...
import { useAssignTask } from "@/hooks/Assignments-Hook"
...
const AssignTask = () => {
  ...
  const [seletctedTask, setSelectedTask] = useState('')
  const [seletctedDeveloper, setSelectedDeveloper] = useState('')
            ...
            {loadingTasks || loadingUsers ? (
              ...
              taskdata?.data
                ...
                ?.map((task: any, index: number) => {
                  const assignedUser = userData?.data?.find((user: any) => user.id === task.assignedTo);
                  ...
                  return (
                    <TableRow key={task.id}>
                      ...
                      <TableCell className="px-6 py-4">{assignedUser?.username}</TableCell>
                      <TableCell className="px-6 py-4">{assignedUser?.email}</TableCell>
                      ...
                      <TableCell className="px-6 py-4">{new Date(task.startDate).toLocaleDateString()}</TableCell>
                      <TableCell className="px-6 py-4">{
                        task.endDate ? new Date(task.endDate).toLocaleDateString() : "_"
                        }</TableCell>
                      <TableCell className="px-6 py-4 text-right">
                       <RemoveAssigned task={task} assignedUser={assignedUser}/>
                      </TableCell>
                    </TableRow>
                  );
                })
